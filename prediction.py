...
#         try:
#             if is_video(curr_vid) or curr_vid.lower().endswith(('.jpg', '.jpeg', '.png')):
...
# def df_face_from_image(image_path, net):
#     from PIL import Image
#     import torchvision.transforms as transforms
#     import torch

#     image = Image.open(image_path).convert("RGB")
#     # Define the preprocessing here (you can customize this):
...
def vids(ed_weight, vae_weight, root_dir="sample_prediction_data", dataset=None, num_frames=15, net=None, fp16=False):
    ...
    # If root_dir is a file (image or video), treat it as single input
    ...
    for curr_vid in files:
        try:
            if is_video(curr_vid) or curr_vid.lower().endswith(('.jpg', '.jpeg', '.png')):
                result, _, count, pred = predict(
                    curr_vid, model, fp16, result, num_frames, net, "uncategorized", count
                )
                print(f"Prediction: {pred[1]} {real_or_fake(pred[0])}")
            else:
                print(f"Invalid file: {curr_vid}")
        except Exception as e:
            print(f"Error processing {curr_vid}: {e}")
    return result
...
def faceforensics(ed_weight, vae_weight, root_dir="FaceForensics/data", dataset=None, num_frames=15, net=None, fp16=False):
    result = set_result()
    count = 0
    accuracy = 0
    model = load_genconvit(config, net, ed_weight, vae_weight, fp16)
    ...
    if os.path.isfile(root_dir):
        label = "REAL" if "original" in root_dir.lower() else "FAKE"
        klass = "original" if label == "REAL" else "DeepFakeDetection"  # fallback
        try:
            result, accuracy, count, _ = predict(
                root_dir, model, fp16, result, num_frames, net, klass, count, accuracy, label
            )
        except Exception as e:
            print(f"Error in single file prediction: {e}")
        return result

    vid_type = ["original_sequences", "manipulated_sequences"]
    result["video"]["compression"] = []
    ffdirs = ["DeepFakeDetection", "Deepfakes", "Face2Face", "FaceSwap", "NeuralTextures"]
    ...
    for v_t in vid_type:
        for dirpath, dirnames, filenames in os.walk(os.path.join(root_dir, v_t)):
            klass = next(filter(lambda x: x in dirpath.split(os.path.sep), ffdirs), "original")
            label = "REAL" if klass == "original" else "FAKE"
            for filename in filenames:
                try:
                    if filename in ff_file:
                        ...
                        compression = "c23" if "c23" in curr_vid else "c40"
                        result, accuracy, count, _ = predict(
                            curr_vid, model, fp16, result, num_frames, net, klass, count, accuracy, label, compression
                        )
                except Exception as e:
                    print(f"An error occurred: {str(e)}")
...
def timit(ed_weight, vae_weight, root_dir="DeepfakeTIMIT", dataset=None, num_frames=15, net=None, fp16=False):
    ...
    if os.path.isfile(root_dir):
        try:
            result, accuracy, count, _ = predict(
                root_dir, model, fp16, result, num_frames, net, "DeepfakeTIMIT", count, accuracy, "FAKE"
            )
        except Exception as e:
            print(f"Error in single file prediction: {e}")
        return result
            ...
            if os.path.isdir(subfolder_path):
                for filename in os.listdir(subfolder_path):
                    if filename.endswith(".avi"):
                        ...
                        try:
                            result, accuracy, count, _ = predict(
                                curr_vid, model, fp16, result, num_frames, net, "DeepfakeTIMIT", count, accuracy, "FAKE"
                            )
                        except Exception as e:
                            print(f"An error occurred: {str(e)}")
...
def dfdc(ed_weight, vae_weight, root_dir="deepfake-detection-challenge/train_sample_videos", dataset=None, num_frames=15, net=None, fp16=False):
    result = set_result()
    count = 0
    accuracy = 0
    model = load_genconvit(config, net, ed_weight, vae_weight, fp16)
    ...
    # Single file case
    if os.path.isfile(root_dir):
        label = "FAKE" if "fake" in root_dir.lower() else "REAL"  # crude fallback logic
        try:
            result, accuracy, count, _ = predict(
                root_dir, model, fp16, result, num_frames, net, "dfdc", count, accuracy, label
            )
        except Exception as e:
            print(f"Error in single file prediction: {e}")
        return result
    ...
    for dfdc in dfdc_data:
        ...
        try:
            if is_video(dfdc_file):
                result, accuracy, count, _ = predict(
                    dfdc_file, model, fp16, result, num_frames, net, "dfdc", count, accuracy, dfdc_meta[dfdc]["label"]
                )
        except Exception as e:
            print(f"An error occurred: {str(e)}")
...
def celeb(ed_weight, vae_weight, root_dir="Celeb-DF-v2", dataset=None, num_frames=15, net=None, fp16=False):
    ...
    # Single file case
    if os.path.isfile(root_dir):
        label = "FAKE" if "synth" in root_dir.lower() else "REAL"
        klass = "Celeb-synthesis" if label == "FAKE" else "Celeb-real"
        try:
            result, accuracy, count, _ = predict(
                root_dir, model, fp16, result, num_frames, net, klass, count, accuracy, label
            )
    ...
    # Original logic for full dataset evaluation
    with open(os.path.join("json_file", "celeb_test.json"), "r") as f:
        cfl = json.load(f)
    ky = ["Celeb-real", "Celeb-synthesis"]
    ...
    for ck in cfl:
        ...
        try:
            if is_video(vid):
                result, accuracy, count, _ = predict(
                    vid, model, fp16, result, num_frames, net, klass, count, accuracy, correct_label
                )
        except Exception as e:
            print(f"An error occurred x: {str(e)}")
...
def main():
    ...
    if os.path.isfile(path):
        result = vids(ed_weight, vae_weight, root_dir=path, dataset=dataset, num_frames=num_frames, net=net, fp16=fp16)
    else:
        result = (
            globals()[dataset](ed_weight, vae_weight, path, dataset, num_frames, net, fp16)
            if dataset in ["dfdc", "faceforensics", "timit", "celeb"]
    ...
    print(f"\n\n--- {perf_counter() - start_time:.2f} seconds ---")
